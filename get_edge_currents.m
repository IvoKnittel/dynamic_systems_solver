...
while j<=len
    ...
    switch device_id
        ...
        case 'bc' %current from collector to base
            ...
            [dummy,dummy2, current]           = ebersmoll2(NaN,u_sink-u_source);
        case 'ec' %current from collector to emitter
            ...
            [dummy, current]                  = ebersmoll2(ubase_emitter,ubase_collector);
        case 'eb' %current from base to emitter
            [dummy, dummy2, dummy3, current]  = ebersmoll2(u_source-u_sink,NaN);    
        case 'bc2'
            [dummy,dummy2, current]           = ebersmoll2(NaN,u_sink-u_source);
        case 'ec2'
            [dummy, current]                  = ebersmoll2(ubase-u_sink,ubase-u_source);
        case 'eb2'
            [dummy, dummy2, dummy3, current]  = ebersmoll2(u_source-u_sink,NaN);
    end
