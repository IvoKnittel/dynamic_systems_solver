...
% Create simulation memory and assign unique ids
% ----------------------------------------------
mem.G=G;
mem.next_unique_id =1;
[mem, nodes_info, edges_info] = assign_unique_ids(mem, nodes_info, edges_info);

% 1. : Same-potential nodes are removed.
% ----------------------------------------------
mem.G.Nodes.info = nodes_info;
mem.G.Edges.info = edges_info;
mem = remove_circuit_nodes(mem, find([nodes_info.disp_only]~=0), 'display_only');

% 2. : Transistors replaced by Ebers-Moll devices.
% ------------------------------------------------
nodes_info = mem.G.Nodes.info';
trans_idx = find([nodes_info.is_trans]~=0);
mem = remove_circuit_nodes(mem, trans_idx(1), 'transistor');

nodes_info = mem.G.Nodes.info';
trans_idx = find([nodes_info.is_trans]~=0);
mem = remove_circuit_nodes(mem, trans_idx, 'transistor');
...
nodes = node_info_to_nodes_init(node_info, edges, comp_params.C_to_ground);
