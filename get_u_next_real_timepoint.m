function [u, t, edge_current_mat, dI_mat] = get_u_next_real_timepoint(time, u, edge_current_mat, dI_mat, adja_mat, R0_mat, cap_mat, L_mat)
...
mask_mat = eps*double(R0_mat==Inf);
maskabs_mat = zeros(size(mask_mat));
maskabs_mat(L_mat==Inf)=Inf;
idx=1:size(mask_mat,2);
for j=1:size(mask_mat,1)
   mask_mat(j,idx<j) = - mask_mat(j,idx<j);
end
edge_current_mat = edge_current_mat + mask_mat; 
